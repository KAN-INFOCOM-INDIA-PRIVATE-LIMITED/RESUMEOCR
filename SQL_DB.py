...
conn = pyodbc.connect('DRIVER={SQL Server};SERVER=DESKTOP-TQHABUO\SQLEXPRESS;DATABASE=ResumeParserDB;Trusted_Connection=yes;')
...
def upload_and_process():
    ...
    if file and allowed_file(file.filename):
        ...
        # Format the address
        formatted_address = extracted_data['address']

        # Insert extracted data into SQL Server
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO ResumeData (name, contact_number, email, skills, education, work_experience, address)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (
            extracted_data['name'],
            ...
            ', '.join(extracted_data['work_experience']),
            formatted_address  # Pass the formatted address string
        ))
        conn.commit()
        cursor.close()
